...
List<Debtor> debtors = new List<Debtor> {
        ...
        static void Main(string[] args)
        {
            List<Debtor> debtors = new List<Debtor>
            {
                // Burada verilən borclular siyahısını daxil edin.
            };

            // 2) rhyta.com ve ya dayrep.com domenlerinde emaili olan borclulari cixartmaq
            var domainDebtors = debtors.Where(d => d.Email.Contains("rhyta.com") || d.Email.Contains("dayrep.com")).ToList();
            PrintDebtors("rhyta.com ve ya dayrep.com domenlerinde emaili olan borclular", domainDebtors);

            // 3) Yashi 26-dan 36-ya qeder olan borclulari cixartmaq
            var ageDebtors = debtors.Where(d => (DateTime.Now.Year - d.BirthDate.Year) >= 26 && (DateTime.Now.Year - d.BirthDate.Year) <= 36).ToList();
            PrintDebtors("Yashi 26-dan 36-ya qeder olan borclular", ageDebtors);

            // 4) Borcu 5000-den cox olmayan borclulari cixartmaq
            var debtLess5000 = debtors.Where(d => d.Debt <= 5000).ToList();
            PrintDebtors("Borcu 5000-den cox olmayan borclular", debtLess5000);

            // 5) Butov adi 18 simvoldan cox olan ve telefon nomresinde 2 ve ya 2-den cox 7 reqemi olan borclulari cixartmaq
            var specialDebtors = debtors.Where(d => d.FullName.Length > 18 && d.PhoneNumber.Count(c => c == '7') >= 2).ToList();
            PrintDebtors("Butov adi 18 simvoldan cox olan ve telefon nomresinde 2 ve ya 2-den cox 7 reqemi olan borclular", specialDebtors);

            // 7) Qishda anadan olan borclulari cixartmaq
            var winterDebtors = debtors.Where(d => d.BirthDate.Month == 12 || d.BirthDate.Month == 1 || d.BirthDate.Month == 2).ToList();
            PrintDebtors("Qishda anadan olan borclular", winterDebtors);
            ...
            var aboveAverageDebtDebtors = debtors.Where(d => d.Debt > averageDebt)
                                                  .OrderBy(d => d.FullName.Split(' ').Last())
                                                  .ThenByDescending(d => d.Debt)
                                                  .ToList();
            PrintDebtors("Borcu, umumi borclarin orta borcunnan cox olan borclular", aboveAverageDebtDebtors);

            // 9) Telefon nomresinde 8 olmayan borclularin yalniz familyasin, yashin ve umumi borcun meblegin cixarmaq
            var noEightInPhoneDebtors = debtors.Where(d => !d.PhoneNumber.Contains('8'))
                                               .Select(d => new { Family = d.FullName.Split(' ').Last(), Age = DateTime.Now.Year - d.BirthDate.Year, d.Debt })
                                               .ToList();
            PrintDebtors("Telefon nomresinde 8 olmayan borclular", noEightInPhoneDebtors);
            ...
            var sameLetterDebtors = debtors.Where(d => d.FullName.GroupBy(c => c).Any(g => g.Count() >= 3))
                                           .OrderBy(d => d.FullName)
                                           .ToList();
            PrintDebtors("Adinda ve familyasinda hec olmasa 3 eyni herf olan borclular", sameLetterDebtors);

            // 13) Borclulardan en coxu hansi ilde dogulubsa hemin ili cixartmaq
            var mostCommonYear = debtors.GroupBy(d => d.BirthDate.Year)
                                        .OrderByDescending(g => g.Count())
                                        .FirstOrDefault()?.Key;
            Console.WriteLine($"Borclulardan en coxu {mostCommonYear}-ci ilde dogulublar");

            // 14) Borcu en boyuk olan 5 borclunun siyahisini cixartmaq
            var top5Debtors = debtors.OrderByDescending(d => d.Debt).Take(5).ToList();
            PrintDebtors("Borcu en boyuk olan 5 borclu", top5Debtors);

            // 15) Butun borcu olanlarin borcunu cemleyib umumi borcu cixartmaq
            var totalDebt = debtors.Sum(d => d.Debt);
            Console.WriteLine($"Butun borcu olanlarin borcu cemi: {totalDebt}");

            // 16) 2ci dunya muharibesin gormush borclularin siyahisi cixartmaq
            var ww2Debtors = debtors.Where(d => d.BirthDate.Year <= 1945).ToList();
            PrintDebtors("2ci dunya muharibesin gormush borclular", ww2Debtors);

            // 18) Nomresinde tekrar reqemler olmayan borclularin ve onlarin borcunun meblegin cixartmaq
            var uniqueNumberDebtors = debtors.Where(d => d.PhoneNumber.Distinct().Count() == d.PhoneNumber.Length)
                                             .Select(d => new { d.FullName, d.Debt })
                                             .ToList();
            PrintDebtors("Nomresinde tekrar reqemler olmayan borclular", uniqueNumberDebtors);
            ...
            var canRepayByBirthday = debtors.Where(d =>
            {
                var monthsUntilBirthday = ((d.BirthDate.Month - DateTime.Now.Month + 12) % 12) + (d.BirthDate.Day >= DateTime.Now.Day ? 0 : 1);
                var totalRepayment = monthsUntilBirthday * 500;
                return d.Debt <= totalRepayment;
            }).ToList();
            PrintDebtors("Oz ad gunune kimi borcun oduyub qurtara bilenlerin siyahisi", canRepayByBirthday);
            ...
            {
                var fullNameLower = d.FullName.ToLower();
                return "smile".All(c => fullNameLower.Count(ch => ch == c) >= 1);
            }).ToList();
            PrintDebtors("Adindaki ve familyasindaki herflerden \"smile\" sozunu yaza bileceyimiz borclular", smileDebtors);
        }

        static void PrintDebtors<T>(string title, List<T> debtors)
        {
            Console.WriteLine($"\n{title}:\n");
